...
> 文档状态：本文是 `20260715_0003` 平台基础交接的历史基线。职位的
> `pending_completion` 公开读取、审核尚未交付以及 `75463aa` 验收数字均已被
> `20260716_0004` 职位补全与核验纵向闭环取代，不得当作当前发布证明。当前行为以
> [README](../README.md)、[平台基础运行手册](./runbooks/platform-foundation.md)和
> `2026-07-16-job-completion-review-vertical-slice.md` 为准；未明确标为 superseded 的
> 平台安全边界仍适用。
...
管理、匹配报告和招聘网站 GUI Agent 仍属于后续工作。
...
本节记录 `20260715_0003` 的历史交付，关于“只查询 `pending_completion`”和审核未
交付的描述已 superseded。当前 `20260716_0004` 增加管理员补全/核验、乐观版本、
不可变 `job_verifications`，学生读取仅限 `verified`；来源重同步不会覆盖已有人审
记录的规范字段。
...
1. **学生职位使用与反馈闭环**：复用当前管理员补全与核验结果，学生侧只消费
   `verified`，并为已关闭或内容变化的职位提供可追踪反馈入口。
