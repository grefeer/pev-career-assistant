...
- [ ] **Step 4: Implement `page_file_hash` + `browse_fetch_urls` with the fallback chain** — per URL, with **hard per-URL caps (one `parallel-fetch` + one `search-interact` maximum across the whole chain, enforced by a counter map)**: primary mode per classification; `PROBE` runs `list` and treats `text_length < 4096` as thin → `search-interact`; `PARALLEL_FETCH` with `page_count == 0` or `used_path` in `{click_fallback_fetch_error}` → `search-interact`; `LIST` with `page_count == 0` → `search-interact`; `status=error` → one retry of the same mode with `--wait 5000`; `status=blocked` → never retried, mapped to `blocked_reason` per the browse output detail (implementer verifies the exact reason key in `browse.py`; the families are login/captcha/anti-bot/unsafe-url/empty-shell). **`spa_shell_empty_no_evidence` is a HARD BLOCK, not a fallback trigger** (browse.py:1186-1197: a 0-char shell is an anti-bot-gated SPA with no rendered UI — verified un-renderable via live probes didi/netease/baidu; status=blocked is returned so the workflow stops instead of burning ~45s on a search-interact that cannot succeed) — a blocked result carrying that marker is terminal, `blocked_reason` forwarded; the recoverable thin-shell case is the separate `spa_shell_no_pagination` path (status ok, evidence preserved). `--cache-mode <cache_mode>` passed only for `list` mode. Each browse call resolves page files against `out_dir` and computes full-hash + length per page via `page_file_hash`; a page file missing on disk is dropped from `page_files` (never crashes). Cache hits (`cached: true`) re-hash the reported `text_path` as the single page file (a cache hit omits `page_files` but includes `text_path` — evidence must not vanish on re-runs). The browse CLI must be built with **raw paths, not `shlex.join`** — Windows `shlex.split(posix=False)` keeps `shlex.join`'s quote characters in the token and browse's argparse then writes evidence into a quote-named directory (every page file silently "missing"); the `dedup_node`/`validate_node` direct-interpolation style round-trips cleanly. The browse JSON parse must accept stderr-suffixed output (browse writes a `[search-interact] ...` audit line to stderr on every search/search-interact invocation, and the runner appends a `[stderr]` tail) — parse the first JSON value via `raw_decode`, never `json.loads` of the whole payload. WeChat URLs return `UrlFetchResult(status="wechat_pending", mode=None, page_files=[])` without any browse call.
