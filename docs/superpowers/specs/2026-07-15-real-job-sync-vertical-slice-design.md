...
`source_key` 唯一；`provider + file_id + sheet_id` 也必须唯一。两个预置来源由应用启动后的显式引导函数或首次同步前的幂等初始化函数写入，不把访问令牌或动态业务数据写入迁移。
...
同步租约不在外部网络调用期间持有数据库行锁。租约设置有限过期时间，并在每页成功提交时刷新；进程崩溃后可以由后续请求接管。租约只负责减少重复工作，数据正确性仍由唯一约束和幂等 upsert 保证。
...
- 已读取数量不超过上游声明 total。
- `has_more=false` 时停止。
- 总页数存在防御性上限；超过时返回 `tencent_protocol_error`。
