...
from skill.job_discovery.runtime.target_evidence import resolve_target_evidence
...
def _playwright_worker_command(url: str, *, collect_links: bool) -> list[str]:
    ...
    command = [
        sys.executable,
        "-m",
        "skill.job_discovery.runtime.playwright_worker",
        "--url",
        url,
    ]
    if collect_links:
...
def _fetch_wechat_article_page(
    ...
    if host != _WECHAT_ARTICLE_HOST:
        return None
    from skill.job_discovery.runtime import wechat as wechat_skill
