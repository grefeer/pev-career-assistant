"""Diagnostic: call run_web_navigation(deeproute) directly and inspect the
result, to find why the supervisor path degrades to 1 candidate while the
standalone extract_rendered_job_evidence baseline gives 21.

Prints: baseline-only candidate count, run_web_navigation evidence_pages /
candidates / error / page_count / navigation_path length.
"""
...
def main() -> None:
    ...
    # 1) Baseline only (deterministic, no LLM).
    ...
    # 2) Full run_web_navigation (baseline + Web Nav Agent).
    ...
    # Dump evidence page text_excerpt heads so we can see what was captured.
