...
- **iter3（HEAD）**：9 题 → C008 C015 Q046 成功（3/9），剩 Q028 Q115 R013 R025 R043 R045。
- **迭代6（d59bc21）**：运行时 _requested_role_seed_urls 增加前端开发工程师角色种子（iguopin 前端搜索页）。
- **iter4（d59bc21）**：6 题 → Q115 R013 R043 成功（3/6）。**累计 25/28 = 89.3%**。
- **迭代7（7925d25）**：_source_allowed_for_goal 不再把泛指"官网"当运营商渠道（Q028 校园官网被误拒的根因）。
- **iter5（7925d25）**：Q028/R025/R045 → 0 成功，但 Q028 转为可解释的已验证负结论（校园源无 AIGC PM 岗，中粮=综合校招、西安图为电气=PDF 非 PM），
  R025（猎聘 广州/深圳 前端，来源硬约束+地域无匹配）、R045（猎聘 AIGC PM，来源硬约束，仅捕到非猎聘 JD）均为可解释安全降级。
- **iter6 启动**：R025/R045 方差重跑（R043 曾靠重跑翻绿，猎聘反爬是间歇性的）。
