"""Moka (app.mokahr.com) certified complete-crawl adapter -- PATH A + PATH B.

Moka career sites are single-page SPAs. Their listing XHR is AES-encrypted
(``{"data": "<base64 ciphertext>", "necromancer": "<key/iv>"}``); we MUST NOT
decrypt it (decryption is circumvention of an anti-scraping measure, out of
scope for an assisted, human-reviewed pipeline). Instead, listings are produced
deterministically from the rendered DOM -- the ``#/job/<uuid>`` hash routes a
human visitor sees -- never by the LLM. The crawl is a single page, terminated
by the fixture-proven ``single_page_mokahr_hash_jobs`` marker plus the reached
...
(see plan Task 4.2 Step 3). Unit tests exercise the fixture-replay driver via
``MokaCrawlDriver.from_fixture``; the production browser path is exercised by
the live smoke.
"""
...
class MokaBlockedError(RuntimeError):
    """A Moka page presented a login/captcha/anti-bot wall; never bypassed."""


class MokaCrawlDriver:
    """Deterministic Moka career-site crawl driver (single page, hash routes).

    Two construction modes:

    * ``from_fixture`` -- fixture replay for unit tests: loads
      ``contract.json``, emits the captured ``sample_listings`` as
      ``RawJobListing`` rows, and terminates with the fixture's
      ``single_page_proof``. No browser is touched.
    * Production -- ``MokaCrawlAdapter.build_driver`` supplies a Playwright
      ``page`` (or a ``page_factory``); ``fetch_listing_page`` navigates the
      landing URL and extracts ``#/job/`` hash routes from the rendered DOM.
    """
    ...
    @classmethod
    def from_fixture(cls, fixture_dir: str | Path) -> "MokaCrawlDriver":
    ...
    def first_listing(self) -> RawJobListing:
        if not self._listings:
            raise IndexError("no Moka listings loaded")
    ...
    def _get_page(self) -> Any:
        if self._page is None:
            if self._page_factory is None:
                raise RuntimeError(
                    "MokaCrawlDriver has no fixture listings and no Playwright page"
                )
...
def _listing_from_sample(sample: dict[str, Any], source_url: str) -> RawJobListing:
    ...
    Moka's job detail page is itself the application page, so ``apply_url``
    carries the job-level ``#/job/`` route and is intentionally distinct from
    the listing ``source_url`` (which ends in ``#/jobs/``).
    """
...
class MokaCrawlAdapter(CompleteCrawlAdapter):
    """Certified complete-crawl adapter for Moka (``app.mokahr.com``)."""
