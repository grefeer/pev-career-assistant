...
| Skill | 产出 | 事实与安全边界 |
| --- | --- | --- |
| `job-discovery` | 公开招聘页面证据、结构化 JD | 仅抓取公开 HTTP(S) 页面；拒绝内网地址、登录、验证码和反爬绕过。 |
...
4. 当任务进入 `waiting_user`，补充城市、岗位、来源或偏好；同一 Run 会在**剩余模型/工具预算**内恢复，而不是新建任务。
5. 审核事实约束的简历修改和面试准备，再由用户自行决定后续投递。

## 关键运行时保证

- MySQL 保存 Run、Plan、Step、Turn、Event 和不可变 Artifact；Redis 不作为不可恢复业务状态。
- 每个 Run 使用跨 Planner、Executor、Verifier 共享的模型调用与工具调用预算。
- 每次工具结果和 Agent 决策仅保存安全摘要，不保存推理过程、密钥或私有上下文。
