...
- [ ] Login-required sites are reported and never crawled via logged-in profiles: after a login wall is observed, `run_skill_script` policy-blocks `login.py`/`crawl.py` and the executor reports the situation instead. Opt-in only via task context `allow_login_state_crawl=true` (default false)
...
| 9 | `backend/app/services/career_skills/registry.py` | The 12 tools across 3 skills |
